...
socket.on("socket-send-room", (data) => {
  ...
  $("#num-of-room").html(
    '<img src="/chat/icon/online.png" style="width: 21px; height: 17px;" /> Online: ' +
      data.numOfOnline
  );
});
...
socket.on("server-send-message", (data) => {
  ...
  $("#wrapper-message").append(
    '<div class="mt-3 mb-3 message-content-receive d-flex"><p class="message-item-receive ml-2 mr-3">' +
      data +
      "</p></div>"
  );
...
$(document).ready(function () {
  ...
  $("#btnSend").click(() => {
    $("#wrapper-message").append(
      '<div class="mt-3 mb-3 message-content d-flex"><p class="message-item ml-3 mr-2">' +
        $("#message").val() +
        "</p></div>"
    );
    $("html, body").animate({ scrollTop: $("#wrapper-message").height() }, 0);
